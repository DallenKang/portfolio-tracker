...
// ⚠️ 判断健不健康【不能只看读到卡片】。2026-08 踩过：读到 100 张、解出 0 笔，
//    系统照样回报 sourceOk:true，於是「一个即将上市的都抓不到」被当成正常状态好几个星期。
//    所以这里要另外数「未来的卡」，让端点可以比对「有未来的卡却解出 0 笔 = 解析坏了」。
function scanIpoCards(html) {
  ...
  for (const c of html.split('<div class="card mb-3').slice(1)) {
    ...
    if (!(yr && mo && dy && cm)) continue;   // 不要求 Board：未上市的卡本来就没有那一栏
    seen++;
...
function parseIpos(html) {
  ...
  for (const c of html.split('<div class="card mb-3').slice(1)) {
    ...
    // ⚠️ Board 只有【已上市】的卡才有，还没上市的没有这一栏。
    //    以前把它列为必要条件 -> 每一个「即将上市」的 IPO 都被丢掉，
    //    而这个功能唯一的用途就是提醒即将上市的。2026-08-03 实测：101 张卡里
    //    yr/mo/dy/cm 全部命中，只有唯一那个即将上市的 UNIPAC(0468) 没有 Board。
    if (!(yr && mo && dy && cm)) continue;
    const iso = `${yr[1]}-${MON3[mo[1]] || "00"}-${dy[1].padStart(2, "0")}`;
    if (iso < today) continue;                    // 只要还没上市的
    const bd = board ? board[1].trim() : "";      // 没这一栏 = 板块还没公布
    if (bd && !bd.includes("ACE") && !bd.includes("Main")) continue; // 有写才筛；没写不能当作不合格
    ...
    rows.push({
      ...
      code: cm[2].trim(),
      name: stripTags(cm[3]),
      market: bd.includes("Main") ? "Main" : (bd.includes("ACE") ? "ACE" : "待公布"),
    ...
    try {
      ...
      if (url.pathname === "/api/ipos") {
        try {
          ...
          const scan = scanIpoCards(page);             // 页面上所有 IPO 卡 + 其中未来的有几张
          if (!scan.seen) return json({ error: "IPO source unavailable (blocked or structure changed)" }, 502);
          const rows = parseIpos(page);
          // 有未来的卡、却一笔都解不出来 = 解析规则跟页面对不上。
          // 这种情况必须报错，不能回报平安 —— 上次就是回报平安才让它坏了好几个星期。
          if (scan.future > 0 && rows.length === 0) {
            return json({ error: `IPO parser broken: ${scan.future} upcoming card(s) on page but 0 parsed`,
                          cardsSeen: scan.seen, futureSeen: scan.future }, 502);
          }
          ...
          // 前端靠它分辨「真的没有新股在排队」(futureSeen=0) vs「有但我们解不出来」。
          return json({ source: KLSE_IPO_URL, version: WORKER_VERSION, sourceOk: true,
                        cardsSeen: scan.seen, futureSeen: scan.future, rows });
        } catch (e) {
